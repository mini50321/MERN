...
export default function EditExhibitionModal({ exhibition, isOpen, onClose, onSuccess }: EditExhibitionModalProps) {
  ...
  const [category, setCategory] = useState(exhibition.category || "Conference");
  const [venueName, setVenueName] = useState((exhibition as any).venue_name || "");
  const [city, setCity] = useState((exhibition as any).city || "");
  ...
  const [imageUrl, setImageUrl] = useState(exhibition.image_url || "");
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState(exhibition.image_url || "");
  const [hashtags, setHashtags] = useState(exhibition.hashtags || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || "Failed to update exhibition");
      }
    } catch (error) {
      console.error("Error updating exhibition:", error);
      setError("An error occurred while updating the exhibition");
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900">Edit Medical Exhibition</h2>
          <button
            onClick={onClose}
